...
public class RedisClientTemplate {
    ...
    private final Logger log = Logger.getLogger(RedisClientTemplate.class);
    ...
    /**
     * @param param:map key 防并发id
     *                  value 随机值，可以不传
     *                  seconds 失效时间，控制时长，可以不传，默认3秒
     * @return flag
     */
    public boolean unComplicate(Map<String, String> param) {
        String key = param.get("key");
        String value = param.get("value");
        int seconds = 3;
        boolean result = true;
        if (null != param.get("seconds")) {
            seconds = Integer.valueOf(param.get("seconds"));
        }
        if (null != param.get("value")) {
            value = key;
        }

        Long flag = this.setnx(key, value);
